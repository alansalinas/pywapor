    ...
    if (Date_LS_or < Date_Start_or and Date_LS_or > Date_End_or):
        ...
        if not os.path.exists(filename_Cloud):
            ...
            if 'shape_dem' not in locals():
                ...
                dest_dem, DEM_resh, geo, proj, ncol, nrow, shape_dem = Create_Example_UTM_File(filename_DEM, 30, UTM_Zone_LS) # was UTM_Zone
...
def Get_LS_Para_Veg(workbook, number, Example_fileName, year, month, day, path_radiance, Apparent_atmosf_transm, cos_zn, dr, vegt_cover_NDVI_max):
    ...
    try:
        if (ws['B%d' % number].value) is not None:
            ...
            NDVI=Reshape_Reproject_Input_data(r'%s' %str(ws['B%d' % number].value),ndvi_fileName_user,Example_fileName)

            water_mask_temp = np.zeros((shape_lsc[1], shape_lsc[0]))
            water_mask_temp[NDVI < 0.0] = 1.0
            save_GeoTiff_proy(lsc, NDVI, ndvi_fileName_user, shape_lsc, nband=1)
    ...
    try:
        if (ws['E%d' % number].value) is not None:
            ...
            water_mask_temp = Reshape_Reproject_Input_data(r'%s' %str(ws['E%d' % number].value), water_mask_temp_fileName, Example_fileName)
    ...
    try:
        if (ws['C%d' % number].value) is not None:
            ...
            Surf_albedo=Reshape_Reproject_Input_data(r'%s' %str(ws['C%d' % number].value),surface_albedo_fileName,Example_fileName)
            save_GeoTiff_proy(lsc, Surf_albedo, surface_albedo_fileName, shape_lsc, nband=1)

        else:

            # use the Landsat reflectance to calculate the surface albede, NDVI
            Surf_albedo = Calc_albedo(Reflect, path_radiance, Apparent_atmosf_transm, Landsat_nr)
...
def Get_LS_Para_Thermal(workbook, number, Example_fileName, year, month, day, water_mask_temp, b10_emissivity, Temp_inst, Rp, tau_sky, surf_temp_offset, Thermal_Sharpening_not_needed, DEM_fileName, UTM_Zone, eact_inst, QC_Map):
    ...
    if ws['D%d' % number].value is None:
        ...
        try:
            ...
            Surface_temp = Reshape_Reproject_Input_data(r'%s' %str(ws['D%d' % number].value),surf_temp_fileName,Example_fileName)
