...
LS_folder = r"D:\Project_FAO\Test_LVL3\PyWAPOR_output"
composite_folder = r"D:\Project_ISRPAL\SWEO_Input\ETLook_output_composite"
output_folder = r"D:\Project_FAO\Test_LVL3\PyWAPOR_output_composite_LVL3"
Date = datetime.datetime(2019, 6, 2)

def main(Date, LS_folder, composite_folder, output_folder):
    ...
    for Diff_day_sorted in Diff_days_sorted:
        ...
        if Continue == 1:
            ...
            for take_day in take_days:
                ...
                if "relMap_LS" not in locals():
                    relMap_LS = np.ones(Array_LS.shape) * np.nan
    ...
    Temp_array = relMap_LS * Array_comp
    ...
    Corr_Frac = Corr_Frac.clip(0.0001, 10000)
    ...
    End_array[End_array>25] = np.nan
    
    DC.Save_as_tiff(os.path.join(output_folder, "et_24_mm_%s.tif" %(date_comp)), End_array, geo_30, 4326)
