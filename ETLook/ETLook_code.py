...
def main(input_folder, output_folder, Date):
    ...
    t_min_bare_filename = os.path.join(output_folder_date, "t_min_bare_%s.tif" %Date_str)
    t_min_full_filename= os.path.join(output_folder_date, "t_min_full_%s.tif" %Date_str)
    ...
    vpd_slope = -0.3
    rs_min = np.where(land_mask==3, 400, 100)
    rcan_max = 1000000
    ...
    z0m_full = 0.04 + 0.01 * (dem_resolution - 30)/(250-30) # 0.1
    ...
    #time2 = time.time() 
    ra_24_toa = ETLook.solar_radiation.daily_solar_radiation_toa_new(sc, decl, iesd, lat, doy, slope, aspect)
    #time3 = time.time()
    ...
    if method == 1:
        ...
        lst_min = ETLook.soil_moisture.maximum_temperature(t_min_bare, t_min_full, vc)
        ...
        if out.t_min_bare == 1:
            DC.Save_as_tiff(t_min_bare_filename, t_min_bare, geo_ex, proj_ex)
        if out.t_min_full == 1:
            DC.Save_as_tiff(t_min_full_filename, t_min_full, geo_ex, proj_ex)            
        if out.w_i == 1:
        ...
        del sc, day_angle, t_air_k_i, vp_i, ad_moist_i, ad_dry_i, ad_i, u_b_i_bare
    ...
    del lai_eff, stress_rad, stress_vpd
    
    # **initial canopy aerodynamic resistance***********************************************************
    
    # find water region using ndvi
    if dem_resolution < 250:
        land_mask = np.where(land_mask==2, 1, land_mask)
        land_mask = np.where(ndvi<0, 2, land_mask)
