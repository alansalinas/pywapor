...
def main(input_folder, output_folder, Date):
    ...
    import pyWAPOR.ETLook_org as ETLook_org
    import pyWAPOR.ETLook_org.outputs as out
    ...
    Lat_filename = os.path.join(input_folder_static, "Lat.tif")
    Lon_filename = os.path.join(input_folder_static, "Lon.tif")
    DEM_filename = os.path.join(input_folder_static, "DEM.tif")
    ...
    Tair_amp_filename = os.path.join(input_folder_static, "Tair_amp_%d.tif" %Date.year)     
    ...
    p_air_0_24 = ETLook_org.meteo.air_pressure_kpa2mbar(p_air_0_24)
    ...
    #t_air_24 = ETLook_org.meteo.disaggregate_air_temperature_daily(t_air_24_coarse, z, z_coarse, lapse)
    ...
    # **ETLook_org.unstable.initial_friction_velocity_daily***********************************************************
    # constants or predefined:
    c1 = 1

    # **ETLook_org.unstable.transpiration***********************************************************
    # constants or predefined:
    iter_h = 3

    # **ETLook_org.resistance.soil_resistance***********************************************************
    # constants or predefined:
    r_soil_pow = -2.1
    r_soil_min = 800


    # **ETLook_org.unstable.initial_sensible_heat_flux_soil_daily***********************************************************
    # constants or predefined:
    ...
    # **ETLook_org.unstable.evaporation***********************************************************
    # constants or predefined:
    r0_grass = 0.23
    ...
    # **effective_leaf_area_index**************************************************
    vc = ETLook_org.leaf.vegetation_cover(ndvi, nd_min, nd_max, vc_pow)
    lai = ETLook_org.leaf.leaf_area_index(vc, vc_min, vc_max, lai_pow)
    lai_eff = ETLook_org.leaf.effective_leaf_area_index(lai)

    vc[np.isnan(QC)] = np.nan
    lai[np.isnan(QC)] = np.nan
    lai_eff[np.isnan(QC)] = np.nan
    if out.vc == 1:
        DC.Save_as_tiff(vc_filename, vc, geo_ex, proj_ex)
    if out.lai == 1:
        DC.Save_as_tiff(lai_filename, lai, geo_ex, proj_ex)
    if out.lai_eff == 1:
        DC.Save_as_tiff(lai_eff_filename, lai_eff, geo_ex, proj_ex)


    #*******TRANSPIRATION COMPONENT****************************************************************

    # **soil fraction**************************************************************
    sf_soil = ETLook_org.radiation.soil_fraction(lai)

    sf_soil[np.isnan(QC)] = np.nan
    if out.sf_soil == 1:
        DC.Save_as_tiff(sf_soil_filename, sf_soil, geo_ex, proj_ex)

    # **atmospheric canopy resistance***********************************************
    iesd = ETLook_org.solar_radiation.inverse_earth_sun_distance(doy)
    sc = ETLook_org.solar_radiation.seasonal_correction(doy)
    day_angle = ETLook_org.clear_sky_radiation.day_angle(doy)
    decl = ETLook_org.solar_radiation.declination(doy)
    lat = ETLook_org.solar_radiation.latitude_rad(lat_deg)
    slope = ETLook_org.solar_radiation.slope_rad(slope_deg)
    aspect = ETLook_org.solar_radiation.aspect_rad(aspect_deg)
    
    #time1 = time.time()
    #ra_24_toa = ETLook_org.solar_radiation.daily_solar_radiation_toa(sc, decl, iesd, lat, slope, aspect)
    #time2 = time.time() 
    ra_24_toa = ETLook_org.solar_radiation.daily_solar_radiation_toa_new(sc, decl, iesd, lat, doy, slope, aspect)
    #time3 = time.time()
    #print("Time method 1 = %s" %(time2-time1))
    #print("Time method 2 = %s" %(time3-time2))
    
    ws = ETLook_org.solar_radiation.sunset_hour_angle(lat, decl)
    ra_24_toa_flat = ETLook_org.solar_radiation.daily_solar_radiation_toa_flat(decl, iesd, lat, ws)
    diffusion_index = ETLook_org.solar_radiation.diffusion_index(trans_24, diffusion_slope, diffusion_intercept)

    # choose one of the two options below
    #ra_24 = ETLook_org.solar_radiation.daily_solar_radiation_flat(ra_24_toa_flat, trans_24)
    ra_24 = ETLook_org.solar_radiation.daily_total_solar_radiation(ra_24_toa, ra_24_toa_flat, diffusion_index, trans_24)
    stress_rad = ETLook_org.stress.stress_radiation(ra_24)
    p_air_24 = ETLook_org.meteo.air_pressure_daily(z, p_air_0_24)
    vp_24 = ETLook_org.meteo.vapour_pressure_from_specific_humidity_daily(qv_24, p_air_24)
    svp_24 = ETLook_org.meteo.saturated_vapour_pressure_average(
                ETLook_org.meteo.saturated_vapour_pressure_maximum(t_air_max_24),
                ETLook_org.meteo.saturated_vapour_pressure_minimum(t_air_min_24))
    vpd_24 = ETLook_org.meteo.vapour_pressure_deficit_daily(svp_24, vp_24)
    stress_vpd = ETLook_org.stress.stress_vpd(vpd_24, vpd_slope)
    stress_temp = ETLook_org.stress.stress_temperature(t_air_24, t_opt, t_min, t_max)
    r_canopy_0 = ETLook_org.resistance.atmospheric_canopy_resistance(lai_eff, stress_rad, stress_vpd, stress_temp, rs_min, rcan_max)
    ...
    if out.lat == 1:
        DC.Save_as_tiff(lat_filename, lat, geo_ex, proj_ex)
    if out.slope == 1:
        DC.Save_as_tiff(slope_filename, slope, geo_ex, proj_ex)
    if out.aspect == 1:
        DC.Save_as_tiff(aspect_filename, aspect, geo_ex, proj_ex)
    if out.ws == 1:
        DC.Save_as_tiff(ws_filename, ws, geo_ex, proj_ex)
    if out.ra_24_toa == 1:
        DC.Save_as_tiff(ra_24_toa_filename, ra_24_toa, geo_ex, proj_ex)
    if out.diffusion_index == 1:
        DC.Save_as_tiff(diffusion_index_filename, diffusion_index, geo_ex, proj_ex)
    if out.ra_24 == 1:
        DC.Save_as_tiff(ra_24_filename, ra_24, geo_ex, proj_ex)
    if out.stress_rad == 1:
        DC.Save_as_tiff(stress_rad_filename, stress_rad, geo_ex, proj_ex)
    if out.p_air_24 == 1:
        DC.Save_as_tiff(p_air_24_filename, p_air_24, geo_ex, proj_ex)
    if out.vp_24 == 1:
        DC.Save_as_tiff(vp_24_filename, vp_24, geo_ex, proj_ex)
    if out.svp_24 == 1:
        DC.Save_as_tiff(svp_24_filename, svp_24, geo_ex, proj_ex)
    if out.vpd_24 == 1:
        DC.Save_as_tiff(vpd_24_filename, vpd_24, geo_ex, proj_ex)
    if out.stress_vpd == 1:
        DC.Save_as_tiff(stress_vpd_filename, stress_vpd, geo_ex, proj_ex)
    if out.stress_temp == 1:
        DC.Save_as_tiff(stress_temp_filename, stress_temp, geo_ex, proj_ex)
    if out.r_canopy_0 == 1:
        DC.Save_as_tiff(r_canopy_0_filename, r_canopy_0, geo_ex, proj_ex)

    # **net radiation canopy******************************************************
    t_air_k_24 = ETLook_org.meteo.air_temperature_kelvin_daily(t_air_24)
    # select one of the below two
    #l_net = ETLook_org.radiation.longwave_radiation_fao_etref(t_air_k_24, vp_24, trans_24)
    l_net = ETLook_org.radiation.longwave_radiation_fao(t_air_k_24, vp_24, trans_24, vp_slope, vp_offset, lw_slope, lw_offset)
    int_mm = ETLook_org.evapotranspiration.interception_mm(P_24, vc, lai, int_max)
    lh_24 = ETLook_org.meteo.latent_heat_daily(t_air_24)
    int_wm2 = ETLook_org.radiation.interception_wm2(int_mm, lh_24)
    rn_24 = ETLook_org.radiation.net_radiation(r0, ra_24, l_net, int_wm2)
    rn_24_canopy = ETLook_org.radiation.net_radiation_canopy(rn_24, sf_soil)
    ...
    if out.t_air_k_24 == 1:
        DC.Save_as_tiff(t_air_k_24_filename, t_air_k_24, geo_ex, proj_ex)
    if out.l_net == 1:
        DC.Save_as_tiff(l_net_filename, l_net, geo_ex, proj_ex)
    if out.int_mm == 1:
        DC.Save_as_tiff(int_mm_filename, int_mm, geo_ex, proj_ex)
    if out.lh_24 == 1:
        DC.Save_as_tiff(lh_24_filename, lh_24, geo_ex, proj_ex)
    if out.int_wm2 == 1:
        DC.Save_as_tiff(int_wm2_filename, int_wm2, geo_ex, proj_ex)
    if out.rn_24 == 1:
    ...
    t_air_k_i = ETLook_org.meteo.air_temperature_kelvin_inst(t_air_i)
    vp_i = ETLook_org.meteo.vapour_pressure_from_specific_humidity_inst(qv_i, p_air_i)
    ad_moist_i = ETLook_org.meteo.moist_air_density_inst(vp_i, t_air_k_i)
    ad_dry_i = ETLook_org.meteo.dry_air_density_inst(p_air_i, vp_i, t_air_k_i)
    ad_i = ETLook_org.meteo.air_density_inst(ad_dry_i, ad_moist_i)
    u_b_i_bare = ETLook_org.soil_moisture.wind_speed_blending_height_bare(u_i, z0m_bare, z_obs, z_b)
    lon = ETLook_org.solar_radiation.longitude_rad(lon_deg)
    ha = ETLook_org.solar_radiation.hour_angle(sc, dtime, lon)
    I0 = ETLook_org.clear_sky_radiation.solar_constant()
    ied = ETLook_org.clear_sky_radiation.inverse_earth_sun_distance(day_angle)
    h0 = ETLook_org.clear_sky_radiation.solar_elevation_angle(lat, decl, ha)
    h0ref = ETLook_org.clear_sky_radiation.solar_elevation_angle_refracted(h0)
    m = ETLook_org.clear_sky_radiation.relative_optical_airmass(p_air_i, p_air_0_i, h0ref)
    rotm = ETLook_org.clear_sky_radiation.rayleigh_optical_thickness(m)
    Tl2 = ETLook_org.clear_sky_radiation.linke_turbidity(wv_i, aod550_i, p_air_i, p_air_0_i)
    G0 = ETLook_org.clear_sky_radiation.extraterrestrial_irradiance_normal(I0, ied)
    B0c = ETLook_org.clear_sky_radiation.beam_irradiance_normal_clear(G0, Tl2, m, rotm, h0)
    Bhc = ETLook_org.clear_sky_radiation.beam_irradiance_horizontal_clear(B0c, h0)
    Dhc = ETLook_org.clear_sky_radiation.diffuse_irradiance_horizontal_clear(G0, Tl2, h0)
    ra_hor_clear_i = ETLook_org.clear_sky_radiation.ra_clear_horizontal(Bhc, Dhc)
    emiss_atm_i = ETLook_org.soil_moisture.atmospheric_emissivity_inst(vp_i, t_air_k_i)
    rn_bare = ETLook_org.soil_moisture.net_radiation_bare(ra_hor_clear_i, emiss_atm_i, t_air_k_i, lst, r0_bare)
    rn_full = ETLook_org.soil_moisture.net_radiation_full(ra_hor_clear_i, emiss_atm_i, t_air_k_i, lst, r0_full)
    h_bare = ETLook_org.soil_moisture.sensible_heat_flux_bare(rn_bare, fraction_h_bare)
    h_full = ETLook_org.soil_moisture.sensible_heat_flux_full(rn_full, fraction_h_full)
    u_b_i_full = ETLook_org.soil_moisture.wind_speed_blending_height_full_inst(u_i, z0m_full, z_obs, z_b)
    u_star_i_bare = ETLook_org.soil_moisture.friction_velocity_bare_inst(u_b_i_bare, z0m_bare, disp_bare, z_b)
    u_star_i_full = ETLook_org.soil_moisture.friction_velocity_full_inst(u_b_i_full, z0m_full, disp_full, z_b)
    L_bare = ETLook_org.soil_moisture.monin_obukhov_length_bare(h_bare, ad_i, u_star_i_bare, t_air_k_i)
    L_full = ETLook_org.soil_moisture.monin_obukhov_length_full(h_full, ad_i, u_star_i_full, t_air_k_i)
    u_i_soil = ETLook_org.soil_moisture.wind_speed_soil_inst(u_i, L_bare, z_obs)
    ras = ETLook_org.soil_moisture.aerodynamical_resistance_soil(u_i_soil)
    raa = ETLook_org.soil_moisture.aerodynamical_resistance_bare(u_i, L_bare, z0m_bare, disp_bare, z_obs)
    rac = ETLook_org.soil_moisture.aerodynamical_resistance_full(u_i, L_full, z0m_full, disp_full, z_obs)
    t_max_bare = ETLook_org.soil_moisture.maximum_temperature_bare(ra_hor_clear_i, emiss_atm_i, t_air_k_i, ad_i, raa, ras, r0_bare)
    t_max_full = ETLook_org.soil_moisture.maximum_temperature_full(ra_hor_clear_i, emiss_atm_i, t_air_k_i, ad_i, rac, r0_full)
    w_i = ETLook_org.soil_moisture.dew_point_temperature_inst(vp_i)
    t_dew_i = ETLook_org.soil_moisture.dew_point_temperature_inst(vp_i)
    
    #time1 = time.time()
    #t_wet_i = ETLook_org.soil_moisture.wet_bulb_temperature_inst(t_air_i, t_dew_i, p_air_i)
    #time2 = time.time()
    t_wet_i = ETLook_org.soil_moisture.wet_bulb_temperature_inst_new(t_air_i, qv_i, p_air_i)
    #time3 = time.time()
    #t_wet_i_new2 = ETLook_org.soil_moisture.wet_bulb_temperature_inst_new2(t_air_i)
    #time4 = time.time()
    #print("Time method 1 = %s" %(time2-time1))
    #print("Time method 2 = %s" %(time3-time2))    
    #print("Time method 3 = %s" %(time4-time3))   
    
    t_wet_k_i = ETLook_org.meteo.wet_bulb_temperature_kelvin_inst(t_wet_i)
    lst_max = ETLook_org.soil_moisture.maximum_temperature(t_max_bare, t_max_full, vc)
    lst_min = ETLook_org.soil_moisture.minimum_temperature(t_wet_k_i, t_air_k_i, vc)
    se_root = ETLook_org.soil_moisture.soil_moisture_from_maximum_temperature(lst_max, lst, lst_min)
    stress_moist = ETLook_org.stress.stress_moisture(se_root, tenacity)
    r_canopy_0 = ETLook_org.resistance.atmospheric_canopy_resistance(lai_eff, stress_rad, stress_vpd, stress_temp, rs_min, rcan_max)
    r_canopy = ETLook_org.resistance.canopy_resistance(r_canopy_0, stress_moist, rcan_max)
    ...
    z_obst = ETLook_org.roughness.obstacle_height(ndvi, z_obst_max, ndvi_obs_min, ndvi_obs_max, obs_fr)
    z_oro = ETLook_org.roughness.orographic_roughness(slope, dem_resolution) #careful - standard res is set to 250 # !!!
    z0m = ETLook_org.roughness.roughness_length(lai, z_oro, z_obst, z_obst_max, land_mask)
    ra_canopy_init = ETLook_org.neutral.initial_canopy_aerodynamic_resistance(u_24, z0m, z_obs)
    ...
    if out.z_obst == 1:
        DC.Save_as_tiff(z_obst_filename, z_obst, geo_ex, proj_ex)
    if out.z_oro == 1:
        DC.Save_as_tiff(z_oro_filename, z_oro, geo_ex, proj_ex)
    if out.z0m == 1:
    ...
    u_b_24 = ETLook_org.meteo.wind_speed_blending_height_daily(u_24, z_obs, z_b)
    ...
    # **ETLook_org.unstable.initial_friction_velocity_daily***********************************************************
    disp = ETLook_org.roughness.displacement_height(lai, z_obst, land_mask, c1)
    u_star_24_init = ETLook_org.unstable.initial_friction_velocity_daily(u_b_24, z0m, disp, z_b)
    ...
    # **ETLook_org.neutral.initial_daily_transpiration***********************************************************
    ad_dry_24 = ETLook_org.meteo.dry_air_density_daily(p_air_24, vp_24, t_air_k_24)
    ad_moist_24 = ETLook_org.meteo.moist_air_density_daily(vp_24, t_air_k_24)
    ad_24 = ETLook_org.meteo.air_density_daily(ad_dry_24, ad_moist_24)
    psy_24 = ETLook_org.meteo.psychrometric_constant_daily(p_air_24, lh_24)
    ssvp_24 = ETLook_org.meteo.slope_saturated_vapour_pressure_daily(t_air_24)
    t_24_init = ETLook_org.neutral.initial_daily_transpiration(rn_24_canopy, ssvp_24, ad_24, vpd_24, psy_24, r_canopy, ra_canopy_init)
    ...
    if out.ad_dry_24 == 1:
        DC.Save_as_tiff(ad_dry_24_filename, ad_dry_24, geo_ex, proj_ex)
    if out.ad_moist_24 == 1:
        DC.Save_as_tiff(ad_moist_24_filename, ad_moist_24, geo_ex, proj_ex)
    if out.ad_24 == 1:
        DC.Save_as_tiff(ad_24_filename, ad_24, geo_ex, proj_ex)
    if out.psy_24 == 1:
        DC.Save_as_tiff(psy_24_filename, psy_24, geo_ex, proj_ex)
    if out.ssvp_24 == 1:
        DC.Save_as_tiff(ssvp_24_filename, ssvp_24, geo_ex, proj_ex)
    if out.t_24_init == 1:
        DC.Save_as_tiff(t_24_init_filename, t_24_init, geo_ex, proj_ex)

    # **ETLook_org.unstable.initial_sensible_heat_flux_canopy_daily***********************************************************
    h_canopy_24_init = ETLook_org.unstable.initial_sensible_heat_flux_canopy_daily(rn_24_canopy, t_24_init)
    ...
    t_24 = ETLook_org.unstable.transpiration(rn_24_canopy, ssvp_24, ad_24, vpd_24, psy_24, r_canopy, h_canopy_24_init, t_air_k_24, u_star_24_init, z0m, disp, u_b_24, z_obs, z_b, iter_h)
    t_24_mm = ETLook_org.unstable.transpiration_mm(t_24, lh_24)
    ...
    # **ETLook_org.radiation.net_radiation_soil***********************************************************
    sf_soil = ETLook_org.radiation.soil_fraction(lai)
    rn_24_soil = ETLook_org.radiation.net_radiation_soil(rn_24, sf_soil)
    ...
    r_soil = ETLook_org.resistance.soil_resistance(se_top, land_mask, r_soil_pow, r_soil_min)
    ...
    ra_soil_init = ETLook_org.neutral.initial_soil_aerodynamic_resistance(u_24, z_obs)
    ...
    # **ETLook_org.meteo.wind_speed_blending_height_daily***********************************************************
    ...
    # **ETLook_org.unstable.initial_friction_velocity_soil_daily***********************************************************

    u_star_24_soil_init = ETLook_org.unstable.initial_friction_velocity_soil_daily(u_b_24, disp, z_b)

    # Save as tiff files
    u_star_24_soil_init[np.isnan(QC)] = np.nan
    if out.u_star_24_soil_init == 1:
        DC.Save_as_tiff(u_star_24_soil_init_filename, u_star_24_soil_init, geo_ex, proj_ex)

    # **ETLook_org.unstable.initial_sensible_heat_flux_soil_daily***********************************************************

    stc = ETLook_org.radiation.soil_thermal_conductivity(se_top)
    vhc = ETLook_org.radiation.volumetric_heat_capacity(se_top, porosity)
    dd = ETLook_org.radiation.damping_depth(stc, vhc)
    g0_bs = ETLook_org.radiation.bare_soil_heat_flux(doy, dd, stc, t_amp_year, lat)
    g0_24 = ETLook_org.radiation.soil_heat_flux(g0_bs, sf_soil, land_mask, rn_24_soil, trans_24, ra_24, l_net, rn_slope, rn_offset)
    e_24_init = ETLook_org.neutral.initial_daily_evaporation(rn_24_soil, g0_24, ssvp_24, ad_24, vpd_24, psy_24, r_soil, ra_soil_init)
    h_soil_24_init = ETLook_org.unstable.initial_sensible_heat_flux_soil_daily(rn_24_soil, e_24_init, g0_24)
    ...
    if out.g0_bs == 1:
        DC.Save_as_tiff(g0_bs_filename, g0_bs, geo_ex, proj_ex)
    if out.g0_24 == 1:
        DC.Save_as_tiff(g0_24_filename, g0_24, geo_ex, proj_ex)
    if out.e_24_init == 1:
        DC.Save_as_tiff(e_24_init_filename, e_24_init, geo_ex, proj_ex)
    if out.h_soil_24_init == 1:
        DC.Save_as_tiff(h_soil_24_init_filename, h_soil_24_init, geo_ex, proj_ex)

    # **ETLook_org.unstable.evaporation***********************************************************

    e_24 = ETLook_org.unstable.evaporation(rn_24_soil, g0_24, ssvp_24, ad_24, vpd_24, psy_24, r_soil, h_soil_24_init, t_air_k_24, u_star_24_soil_init, disp, u_b_24, z_b, z_obs, iter_h)
    e_24_mm = ETLook_org.unstable.evaporation_mm(e_24, lh_24)
    et_24_mm = ETLook_org.evapotranspiration.et_actual_mm(e_24_mm, t_24_mm)
    ...
    # **ETLook_org.unstable.evaporation***********************************************************
    rn_24_grass = ETLook_org.radiation.net_radiation_grass(ra_24, l_net, r0_grass)
    et_ref_24 = ETLook_org.evapotranspiration.et_reference(rn_24_grass, ad_24, psy_24, vpd_24, ssvp_24, u_24)
    et_ref_24_mm = ETLook_org.evapotranspiration.et_reference_mm(et_ref_24, lh_24)
