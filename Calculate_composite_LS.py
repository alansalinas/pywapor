...
def main(output_folder, Startdate, Enddate, latlim, lonlim, LS_WAPOR_folder_in, LS_WAPOR_folder_out, LandCover = "GlobCover", Short_Downwards_Radiation = "MSGCCP", RAW_folder = None):
    ...
    folder_input_ETLook = os.path.join(output_folder, "ETLook_input_composite_LS")
    ...
    if not os.path.exists(T_amplitude_global_temp_filename):
        pyWAPOR.Pre_ETLook.download_file_from_google_drive("1pqZnCn-1xkUC7o1csG24hwg22fV57gCH", T_amplitude_global_temp_filename)
    ...
    for Date in Dates:
        ...
        try:
            ...
            if not os.path.exists(NDVI_file):
                ...
                input_format_NDVI = os.path.join(LS_WAPOR_folder_in, "{yyyy}{mm:02d}{dd:02d}", "NDVI_{yyyy}{mm:02d}{dd:02d}.tif")
                NDVI_comp, geo, proj = Calc_Composite_Indicators_LS(input_format_NDVI, Date, Timestep_composite, method = "mean")
            ...
            if not os.path.exists(ALBEDO_file):
                ...
                input_format_ALBEDO = os.path.join(LS_WAPOR_folder_in, "{yyyy}{mm:02d}{dd:02d}", "ALBEDO_{yyyy}{mm:02d}{dd:02d}.tif")
                ALBEDO_comp, geo, proj = Calc_Composite_Indicators_LS(input_format_ALBEDO, Date, Timestep_composite, method = "mean")
            ...
            if not os.path.exists(SE_root_file):
                ...
                input_format_SE_root = os.path.join(LS_WAPOR_folder_out, "{yyyy}{mm:02d}{dd:02d}", "se_root_{yyyy}{mm:02d}{dd:02d}.tif")
                SE_root_comp, geo, proj = Calc_Composite_Indicators_LS(input_format_SE_root, Date, Timestep_composite, method = "mean")
            ...
            if not os.path.exists(P_file): 
                ...
                P = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format, Date, NDVI_file, "mean")
                P[np.isnan(P)] = 0
            ...
            if not os.path.exists(pair_24_0_file):   
                ...
                Pair_24_sea = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_pair_24_sea, Date, NDVI_file, "mean")
            ...
            if not os.path.exists(qv_24_file):   
                ...
                qv_24 = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_qv_24, Date, NDVI_file, "mean")
            ...
            if not (os.path.exists(Tair_24_file) and os.path.exists(Tair_max_24_file) and os.path.exists(Tair_min_24_file)):
                ...
                tair_24 = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_tair_24, Date, NDVI_file, "max", DEM_file = DEM_file, lapse = -0.006)
                ...
                tair_min_24 = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_tair_min_24, Date, NDVI_file, "max", DEM_file = DEM_file, lapse = 0.0)
                ...
                tair_max_24 = pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_tair_max_24, Date, NDVI_file, "max", DEM_file = DEM_file, lapse = 0.0)
            ...
            if not os.path.exists(wind_24_file):
                ...
                u2_24= pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_u2_24, Date, NDVI_file, "mean")
                u2_24[np.isnan(u2_24)] = 0                       
 
                v2_24= pyWAPOR.Pre_ETLook.Calc_Composite_METEO(input_format_v2_24, Date, NDVI_file, "mean")
...
def Calc_Composite_Indicators_LS(input_format, Date, Timestep_composite, method = "mean"):
    
    # Find dates LS
    Dates_LS = np.array([datetime.datetime.strptime(k, "%Y%m%d").toordinal() for k in os.listdir(os.path.dirname(os.path.dirname(input_format))) if len(k)==8])
    Dates_LS_selected = Dates_LS[np.logical_and(Dates_LS>=Date.toordinal(), Dates_LS<=(Date + datetime.timedelta(days=int(Timestep_composite-1))).toordinal())]
    
    i = 0
    for Date_LS_selected in Dates_LS_selected:
        date_comp = datetime.datetime.fromordinal(Date_LS_selected)
        ...
        if Date_LS_selected == Dates_LS_selected[0]:
