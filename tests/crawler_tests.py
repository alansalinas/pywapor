...
if __name__ == "__main__":
    ...
    session = requests.Session()
    try:
        out_fp = download_url(url, fp, session = None, waitbar = True, headers = None, 
    ...
    test_folder = os.path.join(folder, "test3")
    if not os.path.isdir(test_folder):
        os.makedirs(test_folder)
    for fn in glob.glob(os.path.join(test_folder, "*")):
        os.remove(fn)
    session = requests.Session()
    out_fps = _download_urls(urls, test_folder, session, fps = None, parallel = 0, headers = None,
                                max_tries = 2, wait_sec = 2)
    ref_out_fps = [os.path.join(test_folder, f"{code}?test_{code}_{i}.txt") for i, code in enumerate(codes) if code < 400]
    assert out_fps == ref_out_fps

    codes = [200, 200, 200, 200, 200, 400, 200, 200, 401, 403, 
            404, 200, 200, 500, 501, 200, 200, 200, 200]
    urls = [f"https://httpstat.us/{code}?test_{code}_{i}.txt" for i, code in enumerate(codes)]
    test_folder = os.path.join(folder, "test4")
    if not os.path.isdir(test_folder):
        os.makedirs(test_folder)
    for fn in glob.glob(os.path.join(test_folder, "*")):
        os.remove(fn)
    ...
    codes = [200, 200, 200, 200, 200, 400, 200, 200, 401, 403, 
            404, 400, 200, 200, 500, 501, 500, 200, 200, 200, 200]
    urls = [f"https://httpstat.us/{code}?test_{code}_{i}.txt" for i, code in enumerate(codes)]
    test_folder = os.path.join(folder, "test6")
    if not os.path.isdir(test_folder):
        os.makedirs(test_folder)
    for fn in glob.glob(os.path.join(test_folder, "*")):
        os.remove(fn)
    ...
    fps = [os.path.join(test_folder, f"{i}_{code}.txt") for i, code in enumerate(codes)]
    out_fps = download_urls(urls, test_folder, session = session, fps = fps, parallel = 0, 
                    headers = None, checker_function = checker_function,
                    max_tries = 2, wait_sec = 2, meta_max_tries = 2)
    ...
    codes = [200, 200, 200, 200, 200, 400, 200, 200, 401, 403, 
            404, 400, 200, 200, 500, 501, 500, 200, 200, 200, 200]
    urls = [f"https://httpstat.us/{code}?test_{code}_{i}.txt" for i, code in enumerate(codes)]
    test_folder = os.path.join(folder, "test7")
    if not os.path.isdir(test_folder):
        os.makedirs(test_folder)
    for fn in glob.glob(os.path.join(test_folder, "*")):
        os.remove(fn)
